...
1. Spawn `{{PREFIX}}-animator-godot` with the unit id, its parts, and the timing
   requirements.
...
4. Capture the result:
   ...
   ```
   {{AGENT_DIR}}/scripts/{{PREFIX}}-visual-godot.sh --scene <unit preview scene>
   ```

   Then spawn `{{PREFIX}}-art-director` in `review` mode on the captured frames.
