...
1. **Baseline first.**
   ...
   ```
   {{AGENT_DIR}}/scripts/{{PREFIX}}-sim-godot.sh --balance --seeds 20
   ```
...
3. Spawn `{{PREFIX}}-economy` with the baseline, the corridor, and the intent.
...
6. Re-run the determinism gate. Content changes can alter iteration order:
   ...
   ```
   {{AGENT_DIR}}/scripts/{{PREFIX}}-sim-godot.sh --replay --seed 4242
   ```
7. Close out with `{{PREFIX}}-docs`, recording the final numbers and why.
