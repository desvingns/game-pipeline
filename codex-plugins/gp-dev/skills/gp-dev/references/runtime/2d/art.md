...
2. `.codex/scripts/gp-style-lock.sh --verify` returns `pass:true`.
...
```
.codex/scripts/gp-art-gen.sh --spec art/prompts/<id>.json
```
...
```
bash .codex/scripts/gp-art-gen.sh --spec art/prompts/<id>.json --register <path> --attempt <n>
```
...
```
.codex/scripts/gp-asset-validate.sh --image assets/inbox/<id>.png --tier <A|B>
```
