...
# Keep staging and overwritten versions in the archive; nothing is deleted.
BOOTSTRAP_ARCHIVE=$(gp_archive_dir "$DEST_ROOT" gp-bootstrap)
mkdir -p "$BOOTSTRAP_ARCHIVE"
STAGE=$(mktemp -d "$BOOTSTRAP_ARCHIVE/run.XXXXXX")
        ...
        case "$rel" in
            ...
            AGENTS.md|CLAUDE.md)
                if ! grep -qE 'Generated by game-pipeline|^# Claude adapter$' "$dst"; then
                    echo "Preserved existing $rel; see $STAGE/generated/$rel for pipeline instructions." >&2
                    continue
                fi ;;
        esac
