...
```
ART card
  -> gp-art-prompter        writes art/prompts/<id>.json
  -> gp-art-gen.sh          provider adapter; writes assets/inbox/<id>.png
                                    + <id>.provenance.json
  -> gp-asset-validate.sh   deterministic gate; one JSON line
  -> gp-art-director        multimodal review against the locked sheet
  -> gp-asset-integrator    into the Godot project
```
