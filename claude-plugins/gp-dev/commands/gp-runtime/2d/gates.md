...
Through `gp-runner-godot`, in this order — cheapest and most likely to
fail first, so a broken import does not hide behind a four-minute balance run:

| # | Gate | Command | Applicable when |
|---|---|---|---|
| 1 | build + tests | `gp-runner-godot.sh` | always |
| 2 | determinism | `gp-sim-godot.sh --replay --seed 4242` | `sim/` exists |
| 3 | balance | `gp-sim-godot.sh --balance --seeds 20` | content tables exist |
| 4 | style lock | `gp-style-lock.sh --verify` | the sheet is locked |
| 5 | assets | `gp-asset-validate.sh --image <each>` | assets exist |
| 6 | screenshots | `gp-visual-godot.sh --all` | a shot list exists |
| 7 | APK export | `gp-runner-godot.sh --export` | only on request |
